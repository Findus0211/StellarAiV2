...
const CharacterSelector: React.FC<CharacterSelectorProps> = ({ onSelectCharacter, user, token }) => {
  ...
  const CharacterCard: React.FC<{character: Character}> = ({ character }) => (
     <div
      ...
      className="bg-[var(--background-secondary)] rounded-lg shadow-lg overflow-hidden cursor-pointer group transform hover:scale-105 hover:shadow-[var(--accent-border)]/30 transition-all duration-300 relative"
    >
      ...
      <div className="p-4">
        <h3 className="text-lg font-semibold text-[var(--text-primary)] group-hover:text-[var(--accent-text)] transition-colors">{character.name}</h3>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-4 sm:p-8 bg-gradient-to-b from-[var(--background-primary)] to-[var(--background-secondary)]/80">
        <div className="text-center mb-6">
          <h2 className="text-3xl font-bold text-[var(--text-primary)]">BSD Mode</h2>
          <p className="text-[var(--text-secondary)] mt-2">Choose a character to begin your conversation.</p>
        </div>
        <div className="max-w-xl mx-auto mb-6">
            <input
                ...
                className="w-full px-4 py-3 bg-[var(--background-tertiary)] border border-[var(--border-secondary)] rounded-lg text-[var(--text-primary)] placeholder-[var(--text-secondary)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-border)] transition-shadow"
            />
        </div>
      </div>
      <div className="flex-1 overflow-y-auto p-4 sm:p-8">
        <div className="max-w-7xl mx-auto">
          {isLoading && !favorites && <p className="text-center text-[var(--text-secondary)]">Loading favorites...</p>}
          
          {/* Favorites Section */}
          {user && favoritedCharacters.length > 0 && (
            <div className="mb-12">
              <h3 className="text-2xl font-semibold text-[var(--text-primary)] mb-6 border-l-4 border-[var(--accent-text)] pl-3">Favorites</h3>
          ...
          <div>
            {user && favoritedCharacters.length > 0 && (
              <h3 className="text-2xl font-semibold text-[var(--text-primary)] mb-6 border-l-4 border-[var(--border-secondary)] pl-3">All Characters</h3>
            )}
